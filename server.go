...
func (s *Server) handle(conn net.Conn) {
	...
	c := common.NewConn(s.id, connInit.LocalId, conn, s.msgPool, s.msgReceiver, s.Conns, s.Router, s.handler)
...
func (s *Server) BindBridge(bd BridgeNode) error {
	...
	conn := common.NewConn(s.id, bd.RemoteId(), bd.Conn(), s.msgPool, s.msgReceiver, s.Conns, s.Router, s.handler)
