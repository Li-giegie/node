...
type Router interface {
	// AddRoute 添加路由
	AddRoute(dst, via uint32, hop uint8, unixNano int64, paths []*implrouter.RoutePath) bool
	// RemoveRoute 移除路由
	...
	GetRoute(dst uint32) (*implrouter.RouteEmpty, bool)
	// GetRouteVia 从路由表中dst路由的下一跳
	GetRouteVia(dst uint32) (uint32, bool)
	// RangeRoute 遍历路由表
	RangeRoute(callback func(*implrouter.RouteEmpty) bool)
	// ReroutingHandleFunc 添加重新计算路由处理方法
	ReroutingHandleFunc(f func(dst uint32) (*implrouter.RouteEmpty, bool))
	// Rerouting 重新计算目的路由下一跳，而不是从路由表中取路由
	Rerouting(dst uint32) (*implrouter.RouteEmpty, bool)
}
