...
func NewRouterBFS(protoType uint8, node Node, MaxRouteHop uint8, requestTimeout, clearExistCacheTime time.Duration) Protocol {
	p := &RouterBFS{
		node:           node,
		protoType:      protoType,
...
func (p *RouterBFS) OnConnect(conn iface.Conn) {
	...
	p.once = true
	for _, connect := range p.node.GetAllConn() {
		go func(connect iface.Conn) {
			p.onConnect(connect)
		}(connect)
	}
}
...
func (p *RouterBFS) onConnect(conn iface.Conn) {
	...
	defer cancel()
	res, code, err := conn.RequestType(ctx, p.protoType, p.newPMsg(Action_Query, nil).Encode())
	if err != nil || code != 200 {
		return
	}
	var proto ProtoMsg
...
func (p *RouterBFS) OnMessage(ctx iface.Context) {
...
func (p *RouterBFS) OnClose(conn iface.Conn, err error) {
	id := conn.RemoteId()
...
func (p *RouterBFS) broadcast(m *ProtoMsg, hop uint8, filterIds ...uint32) {
	data := m.Encode()
	p.protoNode.Range(func(id uint32, conn iface.Conn) {
