...
func newProtoNodeContainer() *protoNodeContainer {
	return &protoNodeContainer{
		cache: make(map[uint32]iface.Conn),
	}
}

type protoNodeContainer struct {
	cache map[uint32]iface.Conn
	l     sync.RWMutex
}

func (p *protoNodeContainer) Add(id uint32, conn iface.Conn) {
	p.l.Lock()
	p.cache[id] = conn
	p.l.Unlock()
}
...
func (p *protoNodeContainer) Range(callback func(id uint32, conn iface.Conn)) {
	p.l.RLock()
