...
func serverNode() {
	srv := node.NewServer(scene2.SERVER_ADDR)

	//先发送往服务节点，服务节点转发到client_node2，不推荐此做法发，应该用注册或者直接转发功能实现
	srv.HandleFunc(scene2.SERVER_FORWARD_API, func(ctx *node.Context) {
		// if ... {
		// ctx.ReplyErr(errors.New("..."),[]byte("..."))
		//}
		// todo: ctx.Data() ......

		conn, ok := srv.GetConnect(scene2.CLIENT1_ID)
		if !ok {
			_ = ctx.ReplyErr(node.ErrConnNotExist, nil)
			return
		}
		//req: c1 --> s --> c2 res:c2 --> c1
